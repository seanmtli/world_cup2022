...
td1 <- biv_cups_only_nona[-train_nums, ] %>%
         select(rank_differential, points_differential, offense_differential ,
                   defense_differential,midfield_differential,neutral_location ) %>% 
  mutate(int_col = rep(1,nrow(.))) %>% 
  relocate(int_col)
td2 <- biv_cups_only_nona[-train_nums, ] %>%
         select(offense_differential, 
                   neutral_location ,midfield_differential) %>% 
  mutate(int_col = rep(1,nrow(.))) %>% 
  relocate(int_col)
...
fin_mod <- lm.bp(l1 = home_team_score~ rank_differential + points_differential + offense_differential +
                   defense_differential+midfield_differential+neutral_location ,
                 l2 = away_team_score~ offense_differential + 
                   neutral_location + midfield_differential,
...
```{r}
# ok multiply
exp(as.matrix(td1) %*% as.matrix(fin_mod$beta1))
exp(as.matrix(td2) %*% as.matrix(fin_mod$beta2))
exp(as.matrix(td3) %*% as.matrix(fin_mod$beta3))


rbvpois(1, 4.620, 1.30, 0.0748)
```
