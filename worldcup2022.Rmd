...
#not split... make train data
fin_mod <- lm.bp(l1 = hgoals_new~ 
                   offense_differential + 
                    neutral_location + midfield_differential + shoot_out,
                 l2 = agoals_new~ offense_differential  +
                   neutral_location + midfield_differential + shoot_out,
                 l3 = ~ offense_differential  +
                   neutral_location + midfield_differential + shoot_out, 
                 data = train_data_biv)


# ok multiply
test_home <- fifa_2022[, "country_1_goals"] # placeholder
test_away <- fifa_2022[, "country_2_goals"] # placeholder
...
group_matches <- matches_sched %>% 
  ...
  rename(hometeam = country1, awayteam = coutry2)
...
```{r}
group_matches<-group_matches %>% full_join(qatar_teams, by = c('hometeam'='Team')) %>% 
  full_join(qatar_teams, by = c('awayteam'='Team')) %>% 
  mutate(
     ...
     rank_differential = Rank.x-Rank.y,
  ...
  select(hometeam, awayteam, offense_differential, defense_differential, midfield_differential,
         rank_differential,points_differential,goalkeeper_differential)
```
...
cleaned_group_stage_matches<-group_matches %>% 
  full_join(worldcup_2022_matchesnew, by=c("hometeam"="home_team","awayteam"="away_team"))
