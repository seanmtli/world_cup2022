...
```{r}
match_box_data <- read_csv("data/FIFAallMatchBoxData.csv")
```
...
```{r}
cups_only_nona_new<-inner_join(cups_only_nona, match_box_data, by = c("year"="year",
                                                            "home_team" = "hname",
                                                        "away_team" = "aname",
...
cups_only_nona_new<- cups_only_nona_new%>%
  mutate(possesion_differential = hPossesion-aPossesion,
         SOT_differential = hshotsOnTarget - ashotsOnTarget,
         shots_differential=hshots-ashots,
         yellowcard_differential=hyellowCards-ayellowCards,
         redcard_differential=hredCards-aredCards,
         foul_differential=hfouls-afouls,
         saves_differential=hsaves-asaves)
#merge cups_only_nona with match_box_data
...
set.seed(1)
n <- nrow(cups_only_nona_new)


train_nums <- seq(1, 0.8* nrow(cups_only_nona_new), 1)

#all the X variables for test data
test_data = cups_only_nona_new[-train_nums, ] %>%
  select(rank_differential, offense_differential, defense_differential, midfield_differential,
         points_differential, neutral_location, shoot_out, possesion_differential,
        SOT_differential,shots_differential,yellowcard_differential,redcard_differential,foul_differential,
        saves_differential ) # add cap_diff if using
#predict function needs a mat, makes categorical into dummy vars
mat = model.matrix(~., test_data)
# actual y - values
test_home <- cups_only_nona_new[-train_nums, "home_team_score"] %>%
  pull()
test_away <- cups_only_nona_new[-train_nums, "away_team_score"] %>% 
  pull()

train_data <- cups_only_nona_new[train_nums, ]
...
biv_cups_only_nona <- cups_only_nona_new %>%
...
td1 <- biv_cups_only_nona[-train_nums, ] %>%
         select(rank_differential,offense_differential,  defense_differential, 
                   neutral_location , shoot_out, possesion_differential, 
                SOT_differential, yellowcard_differential, redcard_differential,
                saves_differential) %>% 
...
fin_mod <- lm.bp(l1 = home_team_score~ 
                   rank_differential + offense_differential + defense_differential +
                   neutral_location + shoot_out + possesion_differential +
                SOT_differential + yellowcard_differential +redcard_differential+ 
                saves_differential,
                 l2 = away_team_score~ rank_differential + offense_differential + defense_differential +
                   neutral_location + shoot_out + possesion_differential +
                SOT_differential + yellowcard_differential +redcard_differential+ 
                saves_differential,
                 l3 = ~ rank_differential + offense_differential + defense_differential +
                   neutral_location + shoot_out + possesion_differential +
                SOT_differential + yellowcard_differential +redcard_differential+ 
                saves_differential, 
                 data = train_data_biv)
